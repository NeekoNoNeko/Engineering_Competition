...
manyThresholds = [(30, 100, 15, 127, 15, 127), # generic_red_thresholds -> index is 0 so code == (1 << 0)
...
#        self.mode = self._mode[mode]
    ...
    def catch(self):
        ...
        blobs = img.find_blobs(self.colour, area_threshold=200, margin=10)# type(blob) is list
...
#            return x, y
        ...
        for b in blobs:
            ...
            img.draw_line((160, 120, b[5], b[6]), color=(0, 0, 0), thickness=2)# color is black
        print("\n\n")
...
class IdentifyColorCards:
    ...
    def find(self):
        img = sensor.snapshot().lens_corr(strength=1.5, zoom=1.0)  # 消除镜头鱼眼畸变
        blobs = img.find_blobs(manyThresholds)
        if len(self.CardColour) == 0:
            for b in blobs:
                if b.code() == 1:
                    self.CardColour.append("red")
                if b.code() == 2:
                    self.CardColour.append("green")
                if b.code() == 3:
                    self.CardColour.append("blue")
        elif len(self.CardColour) == 1:
            for b in blobs:
                if b.code() == 1:
                    self.CardColour.append("red")
                if b.code() == 2:
